...
function walk(dir, skip, out = []) {
  ...
  for (const entry of entries) {
    ...
    // A symlink is followed when it resolves to a file; a dangling one is not
    // a violation of anything, so it is skipped rather than crashing the walk.
    try {
...
/**
 * `search` ignores a `g` flag and `lastIndex`. `RegExp.test` does not: a
 * `scan.match` written with `g` would alternate between hit and miss across
 * files and freeze half the repository as the whole of it.
 */
const matches = (pattern, text) => text.search(pattern) !== -1

/**
 * `String.match` without `g` returns one match and its capture groups, which
 * counted as hits turns one occurrence into four. That shape carries `index`
 * and `input`; it is one hit.
 */
function hitsOf(result) {
  if (!result) return []
  if (Array.isArray(result) && result.index !== undefined && result.input !== undefined) return [result[0]]
  return result
}
...
/**
 * A definition is code someone else wrote, and a missing field surfaces as a
 * stack trace three functions deep. Checked once, named once.
 */
function validate(loaded, path) {
  const faults = []
  if (!loaded || typeof loaded !== 'object') faults.push('no default export object')
  else {
    if (typeof loaded.name !== 'string' || !loaded.name) faults.push('`name` must be a non-empty string')
    if (!loaded.rules || typeof loaded.rules !== 'object') faults.push('`rules` must be an object')
    else {
      for (const [name, rule] of Object.entries(loaded.rules)) {
        if (typeof rule?.detect !== 'function') faults.push(`rule ${name}: \`detect\` must be a function`)
        if (rule?.match !== undefined && !isRegExp(rule.match)) faults.push(`rule ${name}: \`match\` must be a RegExp`)
        if (rule?.escapes !== undefined && !(Array.isArray(rule.escapes) && rule.escapes.every(isRegExp))) {
          faults.push(`rule ${name}: \`escapes\` must be an array of RegExp`)
        }
        if (rule?.surface !== 'commits' && rule?.regime !== 'ratchet' && rule?.regime !== 'gate') {
          faults.push(`rule ${name}: \`regime\` must be 'ratchet' or 'gate', or \`surface\` must be 'commits'`)
        }
      }
      const scansFiles = Object.values(loaded.rules).some((rule) => rule?.surface !== 'commits')
      if (scansFiles) {
        if (!Array.isArray(loaded.scan?.dirs) || loaded.scan.dirs.length === 0) faults.push('`scan.dirs` must be a non-empty array')
        if (!isRegExp(loaded.scan?.match)) faults.push('`scan.match` must be a RegExp')
      }
    }
  }
  if (faults.length === 0) return
  console.error(`invalid definition at ${readable(path)}:\n  ${faults.join('\n  ')}`)
  process.exit(2)
}
...
function measure(definition, root) {
  ...
  const files = found
    ...
    .filter((key) => !key.split('/').some((part) => skip.has(part)) && matches(scan.match, key))
  const byRule = {}
  ...
  for (const key of files) {
    ...
    for (const [name, rule] of Object.entries(rules)) {
      if (rule.match && !matches(rule.match, key)) continue
      source ??= readFileSync(join(root, key), 'utf-8')
      let hits = hitsOf(rule.detect(source, key))
      if (rule.escapes?.length) {
        hits = hits.filter((hit) => !rule.escapes.some((escape) => matches(escape, contextOf(hit))))
      }
      if (hits.length > 0) {
...
function measureCommits(rule, range, root) {
  ...
  for (const entry of raw.split(RECORD)) {
    ...
    if (hitsOf(rule.detect(message ?? '')).length > 0) {
...
const hasRatchet = (definition) => Object.values(definition.rules).some((rule) => rule.regime !== 'gate')
...
function update(definition, current, baseline, baselinePath) {
  if (!hasRatchet(definition)) {
    console.log('nothing to freeze: every rule is a gate, and a gate stores no baseline')
    return
  }
  for (const [ruleName, rule] of Object.entries(definition.rules)) {
...
function reportMessage(definition, messagePath) {
  ...
  for (const [name, rule] of Object.entries(definition.rules)) {
    ...
    if (hitsOf(rule.detect(message)).length === 0) continue
    failed = true
...
export async function run(
  ...
  const baseline = hasRatchet(definition) ? loadBaseline(baselinePath, updating) : { byRule: {} }
